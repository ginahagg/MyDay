...
// Thanks to http://stackoverflow.com/questions/3805177/how-to-convert-hex-rgb-color-codes-to-uicolor
    func colorFromHexCode (hexString :String) -> UIColor {
    var cleanString : NSString = hexString.stringByReplacingOccurrencesOfString(@"#", @"")
    if (cleanString.length == 3) {
        cleanString = cleanString.stringWithFormat([@"%@%@%@%@%@%@",
    ...
    if(cleanString.length == 6) {
        ...
        var baseValue:UInt
        var scannerWithString : NSScanner = scannerWithString.cleanString( scanHexInt:baseValue);
    
        red :Float = ((baseValue >> 24) & 0xFF)/255.0f
        green : Float = ((baseValue >> 16) & 0xFF)/255.0f
        blue : Float = ((baseValue >> 8) & 0xFF)/255.0f
        alpha : Float = ((baseValue >> 0) & 0xFF)/255.0f
